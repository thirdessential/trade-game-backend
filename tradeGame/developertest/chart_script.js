// console.log("chart_file_path",chart_file_path)
function drawChart() {
console.log("chart_file_path 1",chart_file_path)
d3.json(chart_file_path).then(function(prices) {
	// d3.csv(chart_file_path).then(function(prices) {
		...
		h = window.innerWidth - margin.top - margin.bottom // window.innerHeight - margin.top - margin.bottom;
		...
		var xAxis = d3.axisBottom()
					    		...
					    		.tickFormat(function(d, index) {
										...
										amPM = hours < 13 ? 'am' : 'pm'
										return hours + ':' + minutes + amPM + ' ' + a.getDate() + ' ' + months[a.getMonth()] + ' ' + a.getFullYear()
									  	} 
		...
		//try click 
		chartBody.selectAll(".candle")
		    .on("click", function(e){
				const {
					...rest
				} = e
				window.postMessage(JSON.stringify(rest), "*")
			})
